...
<script src="//1.www.s81c.com/common/v18/js/forms.js"></script>
    ...
    <script script type="text/javascript" src="js/filterstrecrp.js"></script>
...
<select  id="Input1" title="SEARCH FOR BRAND">
  ...
  <?php
                    ...
                    $query = "SELECT BRAND FROM CTRLSYSTEM.TRECRP GROUP BY BRAND";
...
<select  id="Input3" title="SEARCH FOR DATE">
  <option value="" selected="selected">Date</option>
   <?php
                    require_once('connectsys.php');
                    $query = 'SELECT "DATE" FROM CTRLSYSTEM.TRECRP GROUP BY "DATE"';
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<div class="ibm-columns ibm-center" data-widget="setsameheight" data-items=".ibm-blocklink">
    ...
    <table class="ibm-data-table ibm-grid ibm-altrows  ibm-center ibm-center-block" id="myTable">
            ...
            <tbody>
                ...
                <?php
            require_once('connectsys.php');
            $query = 'SELECT "AREA","BRAND","NOM","DATE","COMMNETS","ING" FROM CTRLSYSTEM.TRECRP  ';
                $stmt = db2_prepare($db2, $query);
      if($stmt){
                 $result = db2_execute($stmt);
                        if (!$result) {

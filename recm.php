...
<nav class="navbar navbar-inverse" id="nav">
        ...
        <div class="collapse navbar-collapse" id="a">
            ...
            <li><a href="viewermfs.php">HOME</a></li>
              <li id="highlightbox"><a href="recm.php">RECORDS</a></li>
              <li><a href="toolsm.php">TOOLS</a></li>
           </ul>
    ...
    <div class="row" id="FILTERSBOX">
    	<div class="col-sm-2">
           ...
           <select id="Input2" placeholder="TYPE" title="SEARCH FOR TYPE" style="font-size: 9px;">
                ...
                <?php
                    ...
                    $query = "SELECT TYPE FROM CTRLSYSTEM.RECMFS GROUP BY TYPE";
        ...
        <div class="col-sm-2">
             ...
             <option value="">AREA</option>
                                <?php
                    require_once('connectsys.php');
                    $query = "SELECT AREA FROM CTRLSYSTEM.RECMFS GROUP BY AREA";
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<select id="Input4" placeholder="BRAND" title="SEARCH FOR BRAND" style="font-size: 9px;">
             ...
             <option value="">BRAND</option>
                  <?php
                    require_once('connectsys.php');
                    $query = "SELECT BRAND FROM CTRLSYSTEM.RECMFS GROUP BY BRAND";
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<select  id="Input5" placeholder="LOCATION" title="SEARCH FOR LOCATION" style="font-size: 9px;">
 ...
 <option value="">LOCATION</option>
                    <?php
                    require_once('connectsys.php');
                    $query = "SELECT LOCATION FROM CTRLSYSTEM.RECMFS GROUP BY LOCATION";
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<select  id="Input7" placeholder="DATE YY/MM/DD" title="SEARCH FOR DATE" style="font-size: 9px;">
 ...
 <option value="">DATE YY/MM/DD</option>
                     <?php
                    require_once('connectsys.php');
                    $query = "SELECT DATE FROM CTRLSYSTEM.RECMFS GROUP BY DATE";
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {

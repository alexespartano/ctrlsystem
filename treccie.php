...
<select id="Input2" title="SEARCH FOR TYPE">
  ...
  <?php
                    ...
                    $query = "SELECT TYPE FROM CTRLSYSTEM.TRECCIE GROUP BY TYPE";
...
<select  id="Input3" title="SEARCH FOR BRAND">
  ...
  <?php
                    require_once('connectsys.php');
                    $query = "SELECT BRAND FROM CTRLSYSTEM.TRECCIE GROUP BY BRAND";
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<select  id="Input5" title="SEARCH FOR DATE">
  <option value="" selected="selected">Date</option>
   <?php
                    require_once('connectsys.php');
                    $query = 'SELECT "DATE" FROM CTRLSYSTEM.TRECCIE GROUP BY "DATE"';
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<div class="ibm-col-6-2  ibm-nospacing">
    <span class="ibm-ind-link ibm-btn-row"><a class="ibm-refresh-link ibm-btn-sec ibm-btn-blue-50" href="#" onClick="window.location.reload()">Refresh</a><a class="ibm-search-link ibm-btn-sec ibm-btn-blue-50" href="#" onclick="filcie()">

...
<script src="//1.www.s81c.com/common/v18/js/forms.js"></script>
    ...
    <script script type="text/javascript" src="js/filterstrecms.js"></script>
...
<div class="ibm-columns ibm-seamless ibm-padding-bottom-0" data-widget="setsameheight" data-items=".ibm-blocklink">
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;
<select  id="Input1" title="SEARCH FOR BRAND">
  <option value="" selected="selected">Brand</option>
  <?php
                    require_once('connectsys.php');
                    $query = "SELECT BRAND FROM CTRLSYSTEM.TRECMS GROUP BY BRAND";
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;
<select id="Input2" title="SEARCH FOR ING RESPONSIBLE">
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;
<select  id="Input3" title="SEARCH FOR DATE">
  <option value="" selected="selected">Date</option>
   <?php
                    require_once('connectsys.php');
                    $query = 'SELECT "DATE" FROM CTRLSYSTEM.TRECMS GROUP BY "DATE"';
                    $stmt = db2_prepare($db2, $query);
                    if($stmt){
                        $result = db2_execute($stmt);
                        if (!$result) {
...
<div class="ibm-col-6-2  ibm-nospacing">
    <span class="ibm-ind-link ibm-btn-row"><a class="ibm-refresh-link ibm-btn-sec ibm-btn-blue-50" href="#" onClick="window.location.reload()">Refresh</a><a class="ibm-search-link ibm-btn-sec ibm-btn-blue-50" href="#" onclick="films()">
...
<div class="ibm-columns ibm-center" data-widget="setsameheight" data-items=".ibm-blocklink">
    ...
    <table class="ibm-data-table     ibm-grid ibm-altrows  ibm-center ibm-center-block"   data-widget="datatable"  style="font-size: 9px" id="myTable" >
            <thead>
                <tr>

                    <th>BRAND</th>
                    <th>AREA</th>
                    <th>ASPECT MEASUREMENT</th>
                    <th>SAMPLE SIZE</th>
                    <th>DATE</th>
                    <th>ING</th>
                     <th>MEASUREMENTS</th>
            ...
            <tbody>
                ...
                <?php
            require_once('connectsys.php');
            $query = 'SELECT "BRAND", "AREA", "ASPMEDI","TAM","DATE", "ING", "MEDIC" FROM CTRLSYSTEM.TRECMS';
                $stmt = db2_prepare($db2, $query);
      if($stmt){
                 $result = db2_execute($stmt);
                        if (!$result) {
